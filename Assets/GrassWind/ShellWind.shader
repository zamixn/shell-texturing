Shader "Custom/ShellWind" {

	Properties
	{
		_NoiseMap("NoiseMap", 2D) = "white" {}
		_WindNoiseMap("Wind Noise", 2D) = "white" {}
		_WindDirectionMap("Wind Direction", 2D) = "white" {}
	}
	...
	SubShader {
		...
		Pass {
			...
			sampler2D _NoiseMap;
			float2 _WindDirChangeSpeed;
			sampler2D _WindNoiseMap;
			sampler2D _WindDirectionMap;
			uniform float4  _WindDirectionMap_ST;
			...
			v2f vp(VertexData v) {
				...
				float windNoise = tex2Dlod(_WindNoiseMap, float4(v.uv, 0, 0)).r;
				float2 uvTiled = frac(v.uv * _WindDirectionMap_ST.xy);//  + _WindDirectionMap_ST.zw;

				i.debug = uvTiled;
				float2 windDirection = tex2Dlod(_WindDirectionMap, float4(uvTiled.rg + _Time.rg * _WindDirChangeSpeed, 0, 0)).rg * 2.0 - float2(1.0, 1.0);
				v.vertex.xz += windDirection * k * _DisplacementStrength * windNoise;
