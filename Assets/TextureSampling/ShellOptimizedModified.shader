Shader "Custom/ShellOptimizedModified" {
	...
	SubShader {
		...
		Pass {
			...
			UNITY_INSTANCING_BUFFER_START(Props)
				UNITY_DEFINE_INSTANCED_PROP(int, _ShellIndex) // This is the current shell layer being operated on, it ranges from 0 -> _ShellCount 
				//UNITY_DEFINE_INSTANCED_PROP(int, _ShellCount) // This is the total number of shells, useful for normalizing the shell index
				//UNITY_DEFINE_INSTANCED_PROP(float, _ShellLength) // This is the amount of distance that the shells cover, if this is 1 then the shells will span across 1 world space unit
				//UNITY_DEFINE_INSTANCED_PROP(float, _Density)  // This is the density of the strands, used for initializing the noise
				//UNITY_DEFINE_INSTANCED_PROP(float, _NoiseMin) // This is the range of possible hair lengths, which the hash then interpolates between 
				//UNITY_DEFINE_INSTANCED_PROP(float, _NoiseMax)
				//UNITY_DEFINE_INSTANCED_PROP(float, _Thickness) // This is the thickness of the hair strand
				//UNITY_DEFINE_INSTANCED_PROP(float, _Attenuation) // This is the exponent on the shell height for lighting calculations to fake ambient occlusion (the lack of ambient light)
				//UNITY_DEFINE_INSTANCED_PROP(float, _OcclusionBias) // This is an additive constant on the ambient occlusion in order to make the lighting less harsh and maybe kind of fake in-scattering
				//UNITY_DEFINE_INSTANCED_PROP(float, _ShellDistanceAttenuation) // This is the exponent on determining how far to push the shell outwards, which biases shells downwards or upwards towards the minimum/maximum distance covered
				//UNITY_DEFINE_INSTANCED_PROP(float, _Curvature) // This is the exponent on the physics displacement attenuation, a higher value controls how stiff the hair is
				//UNITY_DEFINE_INSTANCED_PROP(float, _DisplacementStrength) // The strength of the displacement (very complicated)
				//UNITY_DEFINE_INSTANCED_PROP(float3, _ShellColor) // The color of the shells (very complicated)
				//UNITY_DEFINE_INSTANCED_PROP(float3, _ShellDirection) // The direction the shells are going to point towards, this is updated by the CPU each frame based on user input/movement
			...
			float _NoiseMin; // This is the range of possible hair lengths, which the hash then interpolates between 
			float _NoiseMax;
			...
			v2f vp(VertexData v) {
				...
				float shellHeight = (float)UNITY_ACCESS_INSTANCED_PROP(Props, _ShellIndex) / (float)_ShellCount;
			...
			float4 fp(v2f i) : SV_TARGET{
                ...
                float shellIndex = (float)UNITY_ACCESS_INSTANCED_PROP(Props, _ShellIndex);
                float shellCount = _ShellCount;

				float noiseMapValue = tex2D(_NoiseMap, i.uv).r;
				...
				int outsideThickness = (localDistanceFromCenter) > (_Thickness * ((rand * noiseMapValue) - h));
				...
				fixed4 col = float4(_ShellColor * ndotl * ambientOcclusion, 1.0);
				return col;
			}
